...
class Volume_construct(nn.Module):
    def __init__(self, volume_size, inchannels=320):
        super().__init__()
        self.num_groups = 1
        self.volume_size = volume_size
        self.preconv11 = nn.Sequential(convbn(inchannels, 256, 3, 1, 1, 1),
                                       nn.ReLU(inplace=True),
                                       convbn(256, 64, 3, 1, 1, 1),
                                       nn.ReLU(inplace=True),
                                       nn.Conv2d(64, 32, 3, 1, 1, 1),
                                       nn.ReLU(inplace=True),
                                       nn.Conv2d(32, 16, 3, 1, 1, 1),)
        self.volume12 = nn.Sequential(
...
class Regression(Regression_bone):
    def __init__(self, maxdisp=192, full_shape=None,KL_mode=False, output_disp=48):
        super().__init__(maxdisp,full_shape,KL_mode,output_disp)
...
class feature_extraction(nn.Module):
    def __init__(self, KL_mode=False):
        ...
        self.layer1 = self._make_layer(MobileV1_Residual, 32,  1, 1, 1, 1)# 1/2
        self.layer2 = self._make_layer(MobileV1_Residual, 64,  1, 2, 1, 1)# 1/4
        self.layer3 = self._make_layer(MobileV1_Residual, 128, 1, 1, 1, 1)# 1/4
        self.layer4 = self._make_layer(MobileV1_Residual, 128, 1, 1, 1, 2)# 1/4 
...
class student_main(model_backbone):
    def __init__(self, maxdisp=192, inchannels=320, head_only=False, freeze_head=False, full_shape=None, KL_mode=False, plain_mode=False, output_disp=48):
